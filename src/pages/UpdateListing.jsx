...
const UpdateListing = () => {
  ...
  // useEffect to get the current listing data
  useEffect(() => {
  ...
  const listingFormSubmitHandler = async (e) => {
    ...
    const docRef = await updateDoc(doc(
      db, 'listings', params.listingId), formDataCopy
      )
  ...
  return (
    <main className='max-w-md  px-3 mx-auto'>
      <h1 className='text-3xl font-bold text-center mt-6'>Update Listing</h1>
      ...
      >
        ...
        <button
          type='submit'
          className='w-full px-4 py-3 mt-6 mb-6 text-lg font-semibold text-white bg-blue-700 rounded hover:bg-blue-600 transition ease-in-out duration-200'>Update Listing
        </button>
      </form>
    </main>
  )
